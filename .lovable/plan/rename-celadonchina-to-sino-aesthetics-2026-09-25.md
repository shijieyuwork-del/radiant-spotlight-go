...
- Preserve `青慈有限公司` exactly.
- Rename both brand SVG files and update every reference to their new filenames.
- Set `VITE_SITE_URL=https://sinoaesthetics.com` in all three requested environment files.
- Make the Company Details operator sentence exactly use `SinoAesthetics Limited (青慈有限公司)`.
...
## Completion report
- Provide the exact changed-file list and count.
- List files that could not be updated, if any.
- List every code identifier renamed, or explicitly state none.
- Confirm build and preview status, and confirm nothing was published or deployed.
