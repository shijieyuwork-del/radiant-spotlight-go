    ...
    <section className="border-t border-border/70 bg-card/45 py-14 sm:py-16 md:py-20" aria-labelledby="patient-stories-title">
      <div className="container">
        <div className="grid gap-6 border-b border-border/80 pb-8 md:grid-cols-[0.8fr_1.2fr] md:items-end md:gap-12">
          <div>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-primary"><Quote className="size-4" aria-hidden="true" />{zh ? "真实患者经历" : "Patient stories"}</span>
            <h2 id="patient-stories-title" className="mt-3 font-display text-4xl font-medium leading-[1.03] tracking-tight sm:text-5xl">{zh ? "他们的中国医美旅程" : "Care that travels with you."}</h2>
          </div>
          <p className="max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">{zh ? "已有 400 多位患者信任 CeladonChina 为他们的旅程提供支持。" : "400+ patients have trusted CeladonChina to support their journey."}</p>
        </div>
              ...
              <article key={story.name} className="py-8 first:pt-8 lg:px-7 lg:py-10 lg:first:pl-0 lg:last:pr-0">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{zh ? `${story.age} 岁 · ${story.countryZh}` : `Age ${story.age} · ${story.country}`}</p>
                  </div>
                  <span className="font-display text-5xl leading-none text-primary/15" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span>
                </div>
