...
export default function ConsultationHandoff({ method, context, onBack, onOpenApp }: Props) {
  const { lang } = useAsia();
  ...
  const message = consultationMessage(context);
  ...
  return (
    <section aria-labelledby={`${id}-title`} className="space-y-4 p-4 sm:p-6">
      ...
      <a href={consultationHandoffUrl(method, context)} target={method === "whatsapp" ? "_blank" : undefined} rel={method === "whatsapp" ? "noopener noreferrer" : undefined}
        ...
        {method === "email" ? copy.openEmail : copy.openWhatsapp}<ArrowUpRight className="size-4 shrink-0" aria-hidden="true" />
      </a>
      ...
      <div className="space-y-2">
        <label htmlFor={`${id}-message`} className="text-sm font-semibold text-foreground">{copy.message}</label>
