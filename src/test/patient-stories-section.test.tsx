...
describe("patient story previews and full stories", () => {
  it("keeps the section heading and gives every patient a named full-story action", () => {
    ...
    expect(screen.getByRole("heading", { level: 2, name: "400+ patients trust Sino Aesthetics" })).toBeVisible();
  ...
  it.each(["en", "zh"])("opens every complete %s story with matching patient details", async (lang) => {
    ...
    if (lang === "zh") {
      expect(screen.getByRole("heading", { level: 2, name: "400+ 位患者信任 Sino Aesthetics" })).toBeVisible();
