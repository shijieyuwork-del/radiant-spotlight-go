...
const DemoDoctorDetail = () => {
  ...
  if (!doctor) {
    return <div className="min-h-screen bg-background"><AsiaNavbar /><main className="container py-24 text-center"><p>{lang === "zh" ? "专家资料不存在。" : lang === "ru" ? "Профиль эксперта не найден." : lang === "es" ? "No se encontró el perfil del experto." : "Doctor profile not found."}</p><Link to="/doctors" className="mt-4 inline-block text-primary underline">{lang === "zh" ? "返回专家列表" : lang === "ru" ? "Вернуться к экспертам" : lang === "es" ? "Volver a los expertos" : "Back to doctors"}</Link></main></div>;
  }
        ...
        <main className="container py-8 md:py-12">
          ...
          <nav className="mt-7 grid gap-3 sm:grid-cols-3" aria-label={lang === "zh" ? "相关内容" : lang === "ru" ? "Связанные материалы" : lang === "es" ? "Contenido relacionado: experto, casos y ciudad" : "Related doctor, case and city content"}>
            <Link to={`/cases?city=${encodeURIComponent(doctor.city)}`} className="group rounded-2xl border border-border/70 bg-card p-4 shadow-soft transition hover:border-primary/30">
              <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">{lang === "zh" ? "恢复日记" : lang === "ru" ? "Дневники" : lang === "es" ? "Diarios de pacientes" : "Patient diaries"}</span>
            ...
            <Link to={`/cities/${doctor.city.toLowerCase()}`} className="group rounded-2xl border border-border/70 bg-card p-4 shadow-soft transition hover:border-primary/30">
              <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">{lang === "zh" ? "城市指南" : lang === "ru" ? "Гид по городу" : lang === "es" ? "Guía de la ciudad" : "City guide"}</span>
            ...
            <Link to={`/doctors?city=${encodeURIComponent(doctor.city)}`} className="group rounded-2xl border border-border/70 bg-card p-4 shadow-soft transition hover:border-primary/30">
              <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">{lang === "zh" ? "同城专家" : lang === "ru" ? "Эксперты города" : lang === "es" ? "Expertos cercanos" : "Nearby experts"}</span>
          ...
          <section className="mt-12 md:mt-16">
            <span className="text-xs font-bold uppercase tracking-[0.14em] text-primary">{lang === "zh" ? "相关案例" : lang === "ru" ? "Похожие случаи" : lang === "es" ? "Diarios de pacientes relacionados" : "Related patient diaries"}</span>
