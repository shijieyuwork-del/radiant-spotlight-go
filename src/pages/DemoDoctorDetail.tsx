...
const DemoDoctorDetail = () => {
  ...
  if (!doctor) {
    return <div className="min-h-screen bg-background"><AsiaNavbar /><main className="container py-24 text-center"><p>{lang === "zh" ? "专家资料不存在。" : lang === "ru" ? "Профиль эксперта не найден." : lang === "es" ? "No se encontró el perfil del experto." : "Doctor profile not found."}</p><Link to="/doctors" className="mt-4 inline-block text-brand underline">{lang === "zh" ? "返回专家列表" : lang === "ru" ? "Вернуться к экспертам" : lang === "es" ? "Volver a los expertos" : "Back to doctors"}</Link></main></div>;
  }

  const whatsapp = `https://wa.me/14708613825?text=${encodeURIComponent(`Hi Sino Aesthetics, I’d like to ask about ${doctor.name} and available consultation options.`)}`;
  return (
    <>
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="container py-8 md:py-12">
          <Link to="/doctors" className="mb-5 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" />{lang === "zh" ? "全部专家" : lang === "ru" ? "Все эксперты" : lang === "es" ? "Todos los expertos" : "All doctors"}</Link>
          <section className="grid gap-7 rounded-[2rem] border border-border/70 bg-card p-5 shadow-pop md:grid-cols-[240px_1fr] md:p-8">
            ...
            <div>
              <span className="pill bg-accent text-accent-foreground"><Stethoscope className="size-3.5 text-primary" />{lang === "zh" ? "虚构示例资料" : lang === "ru" ? "Демо-профиль" : lang === "es" ? "Perfil de muestra ficticio" : "Fictional sample profile"}</span>
              <h1 className="mt-4 font-display text-3xl font-semibold tracking-tight md:text-5xl">{doctor.name}</h1>
              <p className="mt-2 text-sm font-bold uppercase tracking-[0.13em] text-brand">{doctor.roleLabel}</p>
              <p className="mt-2 text-lg text-muted-foreground">{doctor.title}</p>
              <Link to={`/cities/${doctor.city.toLowerCase()}`} className="mt-5 flex items-center gap-2 text-sm transition hover:text-brand"><MapPin className="size-4 text-primary" />{doctor.city}, China</Link>
              ...
              <a href={whatsapp} target="_blank" rel="noreferrer" className="cta-primary mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl px-6 text-sm font-bold sm:w-auto sm:rounded-full"><MessageCircle className="size-4" />{lang === "zh" ? "咨询这位专家" : lang === "ru" ? "Спросить об этом эксперте" : lang === "es" ? "Preguntar sobre este experto" : "Ask about this expert"}</a>
            </div>
          </section>

          <nav className="mt-7 grid gap-3 sm:grid-cols-3" aria-label={lang === "zh" ? "相关内容" : lang === "ru" ? "Связанные материалы" : lang === "es" ? "Contenido relacionado: experto, casos y ciudad" : "Related doctor, case and city content"}>
            <Link to={`/cases?city=${encodeURIComponent(doctor.city)}`} className="group rounded-2xl border border-border/70 bg-card p-4 shadow-soft transition hover:border-primary/30">
              <span className="text-label font-bold uppercase tracking-[0.14em] text-brand">{lang === "zh" ? "恢复日记" : lang === "ru" ? "Дневники" : lang === "es" ? "Diarios de pacientes" : "Patient diaries"}</span>
              <span className="mt-1 flex items-center justify-between text-sm font-semibold">{lang === "zh" ? `查看${doctor.city}案例` : lang === "ru" ? `Случаи: ${doctor.city}` : lang === "es" ? `Casos de ${doctor.city}` : `Cases from ${doctor.city}`}<ArrowLeft className="size-4 rotate-180 text-primary transition-transform group-hover:translate-x-0.5" /></span>
            </Link>
            <Link to={`/cities/${doctor.city.toLowerCase()}`} className="group rounded-2xl border border-border/70 bg-card p-4 shadow-soft transition hover:border-primary/30">
              <span className="text-label font-bold uppercase tracking-[0.14em] text-brand">{lang === "zh" ? "城市指南" : lang === "ru" ? "Гид по городу" : lang === "es" ? "Guía de la ciudad" : "City guide"}</span>
              <span className="mt-1 flex items-center justify-between text-sm font-semibold">{lang === "zh" ? `了解${doctor.city}` : lang === "ru" ? `О городе ${doctor.city}` : lang === "es" ? `Explorar ${doctor.city}` : `Explore ${doctor.city}`}<ArrowLeft className="size-4 rotate-180 text-primary transition-transform group-hover:translate-x-0.5" /></span>
            </Link>
            <Link to={`/doctors?city=${encodeURIComponent(doctor.city)}`} className="group rounded-2xl border border-border/70 bg-card p-4 shadow-soft transition hover:border-primary/30">
              <span className="text-label font-bold uppercase tracking-[0.14em] text-brand">{lang === "zh" ? "同城专家" : lang === "ru" ? "Эксперты города" : lang === "es" ? "Expertos cercanos" : "Nearby experts"}</span>
              <span className="mt-1 flex items-center justify-between text-sm font-semibold">{lang === "zh" ? `比较${doctor.city}专家` : lang === "ru" ? `Эксперты: ${doctor.city}` : lang === "es" ? `Comparar expertos de ${doctor.city}` : `Compare ${doctor.city} experts`}<ArrowLeft className="size-4 rotate-180 text-primary transition-transform group-hover:translate-x-0.5" /></span>
            </Link>
          </nav>

          <section className="mt-12 md:mt-16">
            <span className="text-xs font-bold uppercase tracking-[0.14em] text-brand">{lang === "zh" ? "相关案例" : lang === "ru" ? "Похожие случаи" : lang === "es" ? "Diarios de pacientes relacionados" : "Related patient diaries"}</span>
            <h2 className="mt-2 font-display text-3xl font-medium tracking-tight md:text-4xl">{lang === "zh" ? "查看该专科的其他案例" : lang === "ru" ? "Другие случаи по этой специализации" : lang === "es" ? "Explora más casos de esta especialidad" : "Explore more cases in this specialty"}</h2>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">{lang === "zh" ? "这些案例按专科匹配，并不代表由该虚构示例专家完成。真实专家发布后，其本人案例将在这里单独展示。" : lang === "ru" ? "Эти дневники подобраны по специализации и не относятся к вымышленному эксперту. Случаи конкретных экспертов появятся после публикации реальных профилей." : lang === "es" ? "Estos diarios se agrupan por especialidad y no se atribuyen a este experto de muestra ficticio. Los casos vinculados a expertos reales aparecerán aquí una vez que se publiquen sus perfiles." : "These diaries are matched by specialty and are not attributed to this fictional sample expert. Expert-linked cases will appear here after real profiles are published."}</p>
