...
The site uses Cloudflare's country code to apply regional consent. In the EEA, United Kingdom and Switzerland, both tags load only after analytics consent. In other regions they load by default unless the visitor has opted out. If country detection fails, analytics remains off and the consent banner is shown. GA4 receives the sanitized events directly, while GTM receives matching `dataLayer` events for Preview and future integrations. Do not add a second GA4/Google tag inside GTM, a CMS plugin, or the theme; doing so would double-count events.
